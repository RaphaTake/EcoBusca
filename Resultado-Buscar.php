...
if (!empty($cidade)) {
    $query .= " AND cidade = ?"; // Adiciona a condição da cidade se estiver preenchida
    $params[] = $cidade; // Armazena o parâmetro da cidade
}

if (!empty($bairro)) {
    $query .= " AND bairro LIKE ?"; // Adiciona a condição do bairro se estiver preenchida
    $params[] = "%$bairro%"; // Adiciona o parâmetro do bairro com coringas
}
...
if (!empty($params)) {
    mysqli_stmt_bind_param($stmt, str_repeat('s', count($params)), ...$params); // Todos os parâmetros são strings
}
...
// Obter os resultados
$selecionar = mysqli_stmt_get_result($stmt);
